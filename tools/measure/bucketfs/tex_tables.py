...
def arm(row) -> str:
    """How a system is named in a table column."""
    return SYSTEM_LABEL[row["system"]]
...
# The CSVs still carry BucketFS's tuned arm, measured at a one-second
# synchronisation period. From 100 MiB up it reproduced the default arm: the
# synchronisation period governs the directory-scan path, not the upload path,
# so once the payload is large enough to dominate the fixed upload cost the two
# arms agree. The chapter reports one configuration per system, and the default
# is the one it reports.
#
# The two arms do *not* agree on fan-out at 1 KiB and 1 MiB. There the whole
# transfer fits inside BucketFS's ~1 s of fixed upload cost, so the cell is
# decided by whether the writer's own copy is published before or after the
# peers'. The default arm publishes it last (19 of 20 repeats) and fan-out is
# exactly 0.000, the floor of `last - writer`; the tuned arm publishes it first
# and the same cells read 0.30-0.95 s. Neither number measures replication.
# Threat five in the results chapter says as much about the reported rows.
#
# The rows are filtered out here rather than deleted from the CSVs, and the
# count goes into the generated file's header comment so that a reader of the
# data and a reader of the tables can be told apart.
...
def nonconvergence_note(rows, out):
    ...
    for row in bad:
        ...
        key = (row["experiment"], SYSTEM_LABEL[row["system"]],
    ...
    print("\\footnotesize", file=out)
    print("\\setlength{\\tabcolsep}{4pt}", file=out)
    print("\\begin{tabular}{llrlrrp{2.6cm}}", file=out)
    print("\\hline", file=out)
    print("Experiment & System & $N$ & Payload & Deadline & Runs & "
          "Nodes that never received it \\\\", file=out)
    print("\\hline", file=out)
    for key in sorted(tally, key=lambda k: (k[0], k[1], k[2], k[3])):
        experiment, system, nodes, payload, deadline, detail = key
        print(f"{experiment} & {system} & {nodes} & {payload} & "
