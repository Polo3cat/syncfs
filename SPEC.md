...
V13: send order == `removed`, `created`, `modified`. modified → `create` only, ⊥ paired remove
...
T32|.|§I list ctest name `syncfs-update`, ⊥ registered — `ctest` run 4 tests: `syncfs-integration`, `syncfs-performance`, `utils-unit`, `source-unit`. fix §I or add test|I
