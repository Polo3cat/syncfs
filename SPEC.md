...
- Control plane ⊥ reliable. ZMQ PUB drop, silently, ∀ msg whose subscriber queue is full & ∀ msg published before that subscriber finished connecting (slow joiner). ⊥ ack, ⊥ retransmit, ⊥ periodic re-announce, ⊥ reconciliation ∴ 1 dropped `create` == 1 file never synced, permanently. `sndhwm`/`rcvhwm` raise the ceiling (§V.40), ⊥ remove it. ∴ ∀ writer ! wait for ∀ peer subscription before first write (§B.9).
...
T41|.|control plane loses announcements & nothing ever notices. ! either reconcile (periodic digest of `files::list` per peer, refetch what a peer lacks) or make the create path acked. `sndhwm`/`rcvhwm` (§V.40) & the acceptance test waiting for subscriptions (§B.9) only shrink the window|C,V40,B9
...
T43|.|node & file count ceiling now bounded by file descriptors, ⊥ measured. `connections_limit` -1 == `max_open_files()` ∴ ∀ run ! size `RLIMIT_NOFILE` for torrents × peers|T18,T42
