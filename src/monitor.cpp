...
{
    if (fd == -1) {
        auto msg = std::format("Failed to initialize monitor {}", ::strerror(errno));
        throw std::runtime_error(msg);
    }
    w = inotify_add_watch(fd, ".", IN_CLOSE_WRITE | IN_DELETE | IN_DONT_FOLLOW | IN_EXCL_UNLINK);
    if (w == -1) {
        auto msg = std::format("Failed to initialize monitor {}", ::strerror(errno));
        throw std::runtime_error(msg);
    }
...
namespace {
    auto _wait(int fd) -> int
    {
        ...
        if (avail == -1) {
            auto msg = std::format("Failed to poll on inotify fd {}", ::strerror(errno));
            throw std::runtime_error(msg);
        }
        if (avail == 1) {
    ...
    auto _read(int fd) -> std::expected<event_wrap, std::string>
    {
        ...
        if (n == -1) { return std::unexpected(std::format("Failed to read on inotify fd. {}", ::strerror(errno))); }
        if (n == 0) { return std::unexpected("inotify fd closed"); }
        if (n < sizeof(inotify_event)) { return std::unexpected("Read something smaller than an inotify event"); }
        return ev;
    }
}// namespace
...
auto Monitor::read() const -> std::expected<monitor::InotifyEvent, std::string>
{
    return _read(fd).transform([](const event_wrap &e) -> InotifyEvent {
        ...
        ev.name = e.ev()->len == 0 ? std::nullopt : std::make_optional(std::string{ e.ev()->name, e.ev()->len });
        return ev;
    });
    /*
